...
update_fit <- function(pars, model) {
  ...
  if (estH) {
    if (sum(ncol(model$H)) > 1)
      stop("I don't know how to deal with Higher Dimensions Models. Modify the update function.")
  ...
  if (esta1) {
    model$a1[, 1] <- pars[nP1 + seq_len(length(model$a1))]
  }
